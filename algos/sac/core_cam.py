...
class SquashedGaussianMLPActor(nn.Module):
    def __init__(self,
        ...
        robot_dim = obs_space.spaces["robot_state"].shape[0]

        self.extractor = extractor_module()
        feature_dim = output_n(self.extractor, cam_space)

        self.net = mlp([feature_dim + robot_dim] + list(hidden_sizes),
    ...
    def forward(self, obs, deterministic=False, with_logprob=True):
        ...
        net_out = self.net(torch.cat((feat, obs_robot_state), dim=-1))
...
class MLPQFunction(nn.Module):
    def __init__(self,
        ...
        self.q = mlp([feature_dim + robot_dim + act_dim] +
    ...
    def forward(self, obs, act):
        ...
        q = self.q(torch.cat((feat, obs_robot_state, act), dim=-1))
...
class MLPActorCritic(nn.Module):
    ...
    def act(self, obs, deterministic=False):
        obs = {
            ...
            "robot_state": torch.as_tensor(obs["robot_state"], dtype=torch.float32).unsqueeze(dim=0),
        }
